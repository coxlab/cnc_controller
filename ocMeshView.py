...
class Orbiter:
    ...
    def setup_model_view(self):
        ...
        glLoadIdentity()
        gluLookAt(self.translation[0], self.translation[1], self.radius, \
                self.translation[0], self.translation[1], \
                self.translation[2], 0, 1, 0)
...
class OCMeshView(NSOpenGLView):
    def awakeFromNib(self):
        ...
        self.electrodeMatrix = \
                numpy.matrix(numpy.identity(4, dtype=numpy.float64))
    ...
    def keyDown_(self, event):
        ...
        if (event.modifierFlags() & NSControlKeyMask):
            ...
            if c == 'r':
                ...
                self.orbiter.rotation = quaternion.fromEuler( \
                        -numpy.pi / 2, 0, -numpy.pi / 2)
                self.scheduleRedisplay()
            elif c == '#':
                self.orbiter.rotation = quaternion.fromEuler( \
                        -numpy.pi / 2, 0, numpy.pi / 2)
    ...
    def mouseDragged_(self, event):
        if self.leftDown != None:
            x, y = self.convertPoint_fromView_(event.locationInWindow(), None)
            self.orbiter.rotate((self.leftDown[1] - y) * 0.01, \
                    (x - self.leftDown[0]) * 0.01, 0.)
            self.leftDown = (x, y)
            self.scheduleRedisplay()
    ...
    def rightMouseDragged_(self, event):
        if self.rightDown != None:
            x, y = self.convertPoint_fromView_(event.locationInWindow(), None)
            self.orbiter.translate((self.rightDown[0] - x) / 5000., \
                    (self.rightDown[1] - y) / 5000., 0.)
            self.rightDown = (x, y)
            self.scheduleRedisplay()
    ...
    def initGL(self):
        ...
        if len(self.meshFilenames) > 0 and len(self.objs) == 0:
            for i in xrange(len(self.meshFilenames)):
                self.objs.append(self.meshFilenames[i], \
                        self.meshTextureFilenames[i])
                #self.objs[-1].prep_lists()
