#!/usr/bin/env python
from image_proc import canny, hough_line_segments
from scipy.ndimage import imread, map_coordinates, median_filter, watershed_ift
...
def find_electrode(im):
    ...
    lines = hough_line_segments(sparser_edge_im, 1, 1 * 2 * pi / 360., \
            5, 40, 10)

    # Assume that the one of interest is the one that is lowest
    # and most to the right
    best_line = None
...
# takes ~115 ms
#@clockit
def resample_slab(im, line, sample_width=20, sample_step=0.25, \
        over_factor=1.5):
    ...
    (X_native, Y_native) = mgrid[0:e_length:sample_step, \
            -sample_width:sample_width:sample_step]
    ...
    return_transform = lambda c: e_center + (c[1] * sample_step) * \
            e_dir + ((c[0] * sample_step) - sample_width) * e_ortho
...
def estimate_tip_location_in_slab(slab):
    """
    Estimate where the tip is in the slab, assuming
    that the slab is oriented correctly
    """
    ...
    normalized_center_slice = normalized_center_slice - \
            mean(normalized_center_slice[-background_offset:])
...
def electrode_alignment_objective(params, im, line_segment, **kwargs):
    ...
    new_dir = dot(array([[cos(theta), -sin(theta)], [sin(theta), \
            cos(theta)]]), e_dir)
    ...
    (slab, to_image_coords) = resample_slab(im, line_segment, 20, \
            sampling_step, slab_oversample)

    use_watershed = False  # doesn't work correctly
    if use_watershed:
        slab = transform_electrode_image_watershed(slab, None, \
                [int(slab.shape[0] / 2.), int(slab.shape[1] / 2.)])
...
#@clockit
def find_electrode_tip_with_refinement(im, base_im, angle_range, \
        angle_resolution):
    ...
    a = [angle_range[0]]
    a = scipy.optimize.brute(lambda a: \
            electrode_alignment_objective(a, diff_im, l)[0], \
            ranges=(slice(angle_range[0], angle_range[1], angle_resolution), ))
    print "a=", a
...
def find_electrode_tip_from_segment(im, base_im, l, angle_range, nsteps):
    ...
    if not no_optimization:
        ranges = [(angle_range)]
        #print ranges
        angle = scipy.optimize.brute(lambda a_: \
                electrode_alignment_objective(a_, diff_im, l, \
                sampling_step=resampling_size)[0], ranges, (), nsteps, 0, None)
...
if __name__ == "__main__":
    ...
    if True:
        #fns = ("1288193037", "1288193138", \
        #        "1288193185", "1288192975",)
        #base_dir = "/Volumes/vnsl/cncLogs/z_old/" \
        #"1288191881/cameras/49712223528793946/"
        #base_im = double(imread(base_dir + "1288192514" + ".png"))
        base_dir = "test_images/0/"
        base_im = double(imread("background_images/0/0.png"))
        fns = [str(i) for i in xrange(9)]
        for filename in fns:
            # base_dir = "test_images/0/"
            ...
            tip = find_electrode_tip_from_segment(im, base_im, segment, \
                    (-0.5, 0.5), 20)
