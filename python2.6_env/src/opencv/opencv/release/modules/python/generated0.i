...
static PyObject *pycvGetOptimalNewCameraMatrix(PyObject *self, PyObject *args, PyObject *kw)
{
  ...
  const char *keywords[] = { "cameraMatrix", "distCoeffs", "imageSize", "alpha", "newCameraMatrix", "newImageSize", "validPixROI", NULL };
  if (!PyArg_ParseTupleAndKeywords(args, kw, "OOOdO|OO", (char**)keywords, &pyobj_cameraMatrix, &pyobj_distCoeffs, &pyobj_imageSize, &alpha, &pyobj_newCameraMatrix, &pyobj_newImageSize, &pyobj_validPixROI))
    return NULL;
  if (!convert_to_CvMat(pyobj_cameraMatrix, &cameraMatrix, "cameraMatrix")) return NULL;
  if (!convert_to_CvMat(pyobj_distCoeffs, &distCoeffs, "distCoeffs")) return NULL;
  if (!convert_to_CvSize(pyobj_imageSize, &imageSize, "imageSize")) return NULL;
...
#endif
  ERRWRAP(cvGetOptimalNewCameraMatrix(cameraMatrix, distCoeffs, imageSize, alpha, newCameraMatrix, newImageSize, validPixROI));
  Py_RETURN_NONE;
}
...
static PyObject *pycvKMeans2(PyObject *self, PyObject *args)
{
  CvArr* samples;
  ...
  if (!PyArg_ParseTuple(args, "OiOO", &pyobj_samples, &nclusters, &pyobj_labels, &pyobj_termcrit))
    return NULL;
...
#endif
  ERRWRAP(cvKMeans2(samples, nclusters, labels, termcrit));
  Py_RETURN_NONE;
}
...
#endif
  ERRWRAP(cvMatchShapes(object1, object2, method, parameter));
  Py_RETURN_NONE;
}
