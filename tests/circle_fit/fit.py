...
def fit(x,y,z,b,w):
    ...
    def resCircle(p, x, y, z, w):
        cx, cy, cz, nx, ny, nz, r = p
        n = array([nx, ny, nz])
        n = n / norm(n)
        
        u, v, P = project_to_plane(x, y, z, n)
        cu, cv, P2 = project_to_plane(cx, cy, cz, n)#vector.apply_matrix_to_points(P, vector.make_homogeneous(transpose(vstack((cx,cy,cz)))))[:,:2]
        err = sqrt( (u - cu)**2 + (v - cv)**2) + w - r
        return err
    
    n = find_normal(x,y,z)
    params = [0., 0., 0., n[0], n[1], n[2], 180.]
    res = leastsq(resCircle, params, args=(x,y,z,w), ftol=1e-12, xtol=1e-12, maxfev=1000000000)
    cx, cy, cz, nx, ny, nz, r = res[0]
    return cx, cy, cz, r

def fit_circle(x, y, z, b, w):
