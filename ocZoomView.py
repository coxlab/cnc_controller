...
class OCZoomView(NSOpenGLView, ZoomView):
    ...
    def keyDown_(self, event):
        x, y = self.oc_to_gl(self.convertPoint_fromView_( \
    ...
    def rightMouseUp_(self, event):
        ...
        if event.modifierFlags() & NSShiftKeyMask:
            ...
            if i != -1:
                ...
                if self.otherView.selectedZoom >= i:
                    self.otherView.selectedZoom -= 1
        else:
            self.add_zoomed_area(x / self.scale, y / self.scale)
            self.otherView.add_zoomed_area(x / self.otherView.scale, \
                    y / self.otherView.scale)
