...
# when you click File->Quit Python (instead of Shift-Q), python will receive a
# SIGKILL it is impossible to catch this and therefore impossible to clean up
# prior to exit
# so...
# it's VERY important that this program be exited using Shift-Q and
# NOTHING ELSE
# sorry :(
...
class ZoomView(object):
    ...
    def __init__(self):  # , scale=1.):
        ...
        # size of the zoomed view is window coordinates in the form
        # (width, height)
        #self.size = size
        self.imageSize = (1., 1.)
        self.contrast = 1.
    ...
    def draw_zoom(self, index):
        ...
        # self.shader_texture_loc = glGetUniformLocation(self.shader,
        #    "my_color_texture")
        # print "found my_color_texture"
        #
        # print "finding contrast"
        # self.shader_contrast_loc = glGetUniformLocation(self.shader,
        #    "contrast")
        ...
        if key in self.keyBindings.keys():
            try:
                ...
                raise AttributeError( \
                        ...
                        "glZoomView has no function %s" \
                        % (key, self.keyBindings[key][0]))
            # pass on location of keydown?
            if self.keyBindings[key][1] == True:
                func(x, y)
            else:
                func()
        return
    ...
    def process_active_mouse_motion(self, x, y):
        ...
        self.zooms[self.selectedZoom]['x'] = \
                self.zooms[self.selectedZoom]['x'] + \
                dX / self.zooms[self.selectedZoom]['z']
        ...
        self.zooms[self.selectedZoom]['y'] = \
                self.zooms[self.selectedZoom]['y'] + \
                dY / self.zooms[self.selectedZoom]['z']
    ...
    def load_texture_from_string(self, imageData):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE8, self.imageSize[0], \
                self.imageSize[1], 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, \
                self.imageData)
        glBindTexture(GL_TEXTURE_2D, 0)
    ...
    def load_image(self, imageFilename):
        ...
        self.scale = max(glutSize[0] / float(self.imageSize[0]), \
                glutSize[1] / float(self.imageSize[1]))
    ...
    def _image_to_gl(self, x, y):
        return x * 2. / self.imageSize[0] - 1., \
                y * -2. / self.imageSize[1] + 1.

    def _gl_to_image(self, x, y):
        return (x + 1.) / 2. * self.imageSize[0], \
                (y + 1) / -2. * self.imageSize[1]
    ...
    def add_zoomed_area(self, x=None, y=None, z=None, c=None):
        ...
        if z == None:
            if len(self.zooms) != 0:
                ...
                z = 1.
        if c == None:
            c = self._defaultZoomColors[len(self.zooms) \
                    % len(self._defaultZoomColors)]
