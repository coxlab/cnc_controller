...
def canny(image, high_threshold, low_threshold):
    ...
    quantized_angle = numpy.around(3 * \
            (grad_angle + numpy.pi) / (numpy.pi * 2))
    ...
    canny_edges = ndimage.binary_dilation(high, \
            structure=numpy.ones((3, 3)), iterations=-1, mask=low)
...
def hough_line_segments(input_im, rho, theta, threshold, min_length, max_gap):
    ...
    result = cv.HoughLines2(cvim, cv.CreateMemStorage(), \
            cv.CV_HOUGH_PROBABILISTIC, rho, theta, threshold, \
            min_length, max_gap)
