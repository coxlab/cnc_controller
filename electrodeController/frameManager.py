...
class FrameManager:
    ...
    def add_transformation_matrix(self, fromFrame, toFrame, tMatrix):
        ...
        if toIndex - fromIndex != 1:
            ...
            raise ValueError(
                'Invalid from and to frames(%s, %s)' % (fromFrame, toFrame))
...
def test_frame_manager():
    ...
    if randomTransforms:
        m = vector.transform_to_matrix(
            tR * (random.rand(
            ) - 0.5), tR * (
                random.rand() - 0.5), tR * (random.rand() - 0.5),
    ...
    startingPoint = matrix([[10 * (random.rand(
    ) - 0.5), 10 * (random.rand() - 0.5), 10 * (random.rand() - 0.5), 1.]])
    points = {}
