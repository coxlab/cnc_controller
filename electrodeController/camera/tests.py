...
def test_stereo_localization_repeatability(camIDs, gridSize, gridBlockSize, calibrationDirectory='../calibrations', frameDirectory=None):
    ...
    def plot_localization(ax, cam, color):
        ...
        # scale axes 1:1
        ox, oy, oz = ax.get_xlim3d(
        ).copy(), ax.get_ylim3d().copy(), ax.get_zlim3d().copy()
        rmax = max(
            (abs(pylab.diff(ox)), abs(pylab.diff(oy)), abs(pylab.diff(oz))))
        ox = (
            ox - pylab.mean(ox)) / abs(pylab.diff(ox)) * rmax + pylab.mean(ox)
        oy = (
            oy - pylab.mean(oy)) / abs(pylab.diff(oy)) * rmax + pylab.mean(oy)
        oz = (
            oz - pylab.mean(oz)) / abs(pylab.diff(oz)) * rmax + pylab.mean(oz)
...
def test_stereo_localization(camIDs, gridSize, gridBlockSize, calibrationDirectory='../calibrations', frameDirectory=None):
    ...
    while keep_capturing:
        ...
        for c in corners:
            ax.scatter([c[0]], [c[1]], [c[2]], c=ptColors[
                       divmod(ptIndex, len(ptColors))[1]])
            # scale axes 1:1
            ox, oy, oz = ax.get_xlim3d(
            ).copy(), ax.get_ylim3d().copy(), ax.get_zlim3d().copy()
            rmax = max((abs(
                pylab.diff(ox)), abs(pylab.diff(oy)), abs(pylab.diff(oz))))
            ox = (ox - pylab.mean(ox)) / abs(pylab.diff(ox)) * \
                rmax + pylab.mean(ox)
            oy = (oy - pylab.mean(oy)) / abs(pylab.diff(oy)) * \
                rmax + pylab.mean(oy)
            oz = (oz - pylab.mean(oz)) / abs(pylab.diff(oz)) * \
                rmax + pylab.mean(oz)
            ax.set_xlim3d([ox[0], ox[1]])
            ax.set_ylim3d([oy[0], oy[1]])
            ax.set_zlim3d([oz[0], oz[1]])
...
def test_camera_pair(camIDs, gridSize, gridBlockSize, calibrationDirectory='../calibrations', frameDirectory=None):
    ...
    pylab.subplot(224)
    pylab.imshow(
        abs(aGrid / mGrid - 1.), interpolation='nearest', origin='lower')
    pylab.title('Ratio')
    pylab.colorbar()
    ...
    if test[0] == 'c':
        ...
        if test[1] == 'l':
            ...
            test_stereo_localization_repeatability(
                (49712223528793951, 49712223528793946), gridSize, gridBlockSize, frameDirectory=frameDir)
        else:
            print "unknown stereo test"
    elif test[0] == 'f':
        if test[1] == 'l':
            test_file_camera(
                49712223528793951, frameDir, gridSize, gridBlockSize)
        elif test[1] == 'r':
            test_file_camera(
                49712223528793946, frameDir, gridSize, gridBlockSize)
        else:
            print "unknown camera"
    else:
        test_camera_pair(
            (49712223528793951, 49712223528793946), gridSize, gridBlockSize)
