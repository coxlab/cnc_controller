...
# clean up the iso bus in case the previous time this program was open it
# crashed
...
class DC1394Camera(camera.Camera, pydc1394.Camera):
    ...
    def configure(self):
        ...
        # ps = min((ps + ps % u, s))/2
        # print "Calculated Packet Size %i, N Packets %i, for fps %.3f" % (ps,
        # NP, fps)
    ...
    def capture_frame(self):
        ...
        self.configure()
        i = self.shot(
        ).reshape((1040, 1392))  # .reshape((1040,1392)).astype('uint8')
        self.stop()
