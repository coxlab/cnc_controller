...
def fit_3d_line(pts, ts):
    ...
    # method to fix flipping the direction of the line (especially when m is
    # parallel to an axis)
    if ts[1] > ts[0]:
        coarseM = pts[1] - pts[0]
    elif ts[0] > ts[1]:
    ...
    angle = numpy.arccos(
        numpy.dot(coarseM, m))  # both vectors were previously normalized
    if angle > (numpy.pi / 2.):
        m *= -1.

    # reassign the origin so that the given 'ts' (locations on the line) are
    # correct
    newOs = []
