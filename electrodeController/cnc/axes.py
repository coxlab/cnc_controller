...
class Axes(IPSerialBridge):
    ...
    def poll_command(self, command, axis=None, value=""):
        ...
        if axis == None:
            ...
            r[axis] = self.send(
                "%i%s%s" % (self.axes[axis], command, value)).rstrip()
        return r
    ...
    def set_acceleration(self, value, axis=None):
        ...
        for (k, v) in m.iteritems():
            if value > v:
                raise ValueError(
                    'new accel %.4f exceeds max %.4f for axis %s' % (value, v, k))
    ...
    def set_deceleration(self, value, axis=None):
        ...
        for (k, v) in m.iteritems():
            if value > v:
                raise ValueError(
                    'new decel(%.4f) exceeds max(%.4f) for axis %s' % (value, v, k))
    ...
    def set_velocity(self, value, axis=None):
        ...
        for (k, v) in m.iteritems():
            if value > v:
                raise ValueError(
                    'new vel %.4f exceeds max %.4f for axis %s' % (value, v, k))
