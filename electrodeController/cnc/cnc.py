...
class FiveAxisCNC:
    ...
    """
    def __init__(self):
        self.linearAxes = Axes(cfg.cncLinearAxesIP, cfg.cncLinearAxesPort,
                               cfg.cncLinearAxes, cfg.serialConnectionTimeout)
        # configure
    ...
    def configure(self):
        ...
        self.linearAxes.send(
            "3EP", 1)  # enter 'program mode' (automatically stored to non-volitile memory)
        self.linearAxes.configure_axis('x', cfg.xAxisConfig)
        self.linearAxes.configure_axis('y', cfg.yAxisConfig)
        self.linearAxes.configure_axis('z', cfg.zAxisConfig)
        ...
        self.headAxes.send(
            "3EP", 1)  # enter 'program mode' (automatically stored to non-voltile memory)
        self.headAxes.configure_axis('w', cfg.wAxisConfig)
    ...
    def motion_done(self):
        ...
        #  1 : axis 2
        #  2 : axis 3
        # print 'controller status:', ord(linearStatus[0]) & 0x07,
        # ord(headStatus[0]) & 0x07
    ...
    def measure_tip_path(self, tipLocations, wPositions):
        ...
        """
        self.pathParams = fit_3d_line(
            tipLocations, wPositions)  # [x0, y0, z0, a, b, c]
    ...
    def calculate_point_rotation(self, deltaAngle, speed, tipOffset):
        ...
        the x, z and b axes
        """
        currArmLength = self.armLength - float(
            self.headAxes.get_position('w')['w']) + tipOffset
    ...
    def calculate_arm_length(self, tipLocations, angles, wPositions):
        ...
        cfg.cncLog.info(
            "measuring arm length with (wPositions, angles, tipLocations):")
        cfg.cncLog.info(wPositions)
        cfg.cncLog.info(angles)
        cfg.cncLog.info(tipLocations)

        # using rayfit
        rotCen, rotNorm, rotRadii = measure_rotation_plane(
            tipLocations, angles, wPositions)
