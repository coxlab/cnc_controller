...
class Controller:
    def __init__(self):
        # construct cameras
        if cfg.fakeCameras is True:
            self.cameras = camera.stereocamera.StereoCamera(
                cfg.leftCamID, cfg.rightCamID, camera.filecamera.FileCamera)
        else:
            self.cameras = camera.stereocamera.StereoCamera(
                cfg.leftCamID, cfg.rightCamID,
                camera.dc1394camera.DC1394Camera)
        ...
        self.fManager = frameManager.FrameManager(
            ['skull', 'tricorner', 'camera', 'cnc'])
    ...
    def load_animal(self, animalCfgFile):
        ...
        self.fManager.add_transformation_matrix(
            'skull', 'tricorner',
            numpy.matrix(numpy.loadtxt(cfg.skullToTCMatrixFile)))

        # test route TODO it might be nice to fix this,
        # when tc points are loaded prior to skull, no points get to meshview
    ...
    def load_log(self, logDir):
        ...
        if cfg.fakeCameras:
            ...
            lFileList = [lCamDir + f for f in os.listdir(
                lCamDir) if '.png' in f]
            rCamDir = logDir + '/cameras/' + str(cfg.rightCamID) + '/'
            rFileList = [rCamDir + f for f in os.listdir(
                rCamDir) if '.png' in f]
        ...
        if os.path.exists(stt):
            self.fManager.add_transformation_matrix(
                'skull', 'tricorner', numpy.matrix(numpy.loadtxt(stt)))
            numpy.savetxt(self.logDir + '/skull_to_tricorner',
                          self.fManager.get_transformation_matrix('skull', 'tricorner'))
        ...
        if os.path.exists(rcpts):
            ...
            if not all(self.cameras.get_located()):
                ...
                if lr[1] == True and rr[1] == True:
                    ...
                    raise Exception(
                        "Cameras failed to localize when trying to load registerCameraPts")
        ...
        if os.path.exists(mppts):
            ...
            if not self.fManager.test_route('skull', 'camera'):
                raise Exception(
                    "attempting to add path points with an incomplete frame stack")
    ...
    def register_cameras(self, ptsInCamera):
        ...
        cfg.framesLog.info(
            'Registering cameras with points in (tcFrame, camera):')
    ...
    def measure_tip_path(self, ptsInCamera, wPositions):
        ...
        cfg.cncLog.info('Found tip path params: %+.2f %+.2f %+.2f %.2f %.2f %.2f' % (
            pathParams[0], pathParams[1], pathParams[2], pathParams[3], pathParams[4], pathParams[5]))
    ...
    def register_cnc(self, ptsInCamera, angles, wPositions):
        ...
        armLength = self.cnc.calculate_arm_length(
            ptsInCamera, angles, wPositions)
        ...
        camToCNC = vector.calculate_rigid_transform(
            ptsInCamera[:3], ptsInCNC[:3])  # TODO check that this is correct
